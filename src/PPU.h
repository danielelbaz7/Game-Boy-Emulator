...
private:
    ...
    void DrawBackground(uint32_t *scanline);
    void DrawWindow(uint32_t *scanline);
    void DrawSprites(uint32_t *scanline);
    ...
    [[nodiscard]] bool spritesEnabled() const { return ((((Read(0xFF40) & 0x01)) == 1) ? true : false); }
